...
const zstdTransformContent = {
	...
	async transform(chunk, controller) {
		...
		while (true) {
			...
			while (true) {
				if (this.inBufferData.pos.value >= this.inBuffer.size) {
					break;
				}
				...
				this.lastReturn = wasm._ZSTD_decompressStream(
					this.ctxPtr,
					this.outBufferData.ptr,
					this.inBufferData.ptr,
				);
				if (wasm._ZSTD_isError(this.lastReturn)) {
					controller.error(
						new Error(
							`Zstd error:\n[${this.lastReturn}] ${wasm.UTF8ToString(wasm._ZSTD_getErrorName(this.lastReturn))}`,
						),
					);
					return;
				}
